...
Last Update: 2026-05-22 (Unified Dynamic Database Sync & Recovery Fix)
"""
...
def register():
    ...
    try:
        ...
        wa = format_whatsapp_number(request.form.get('no_whatsapp', ''))
        no_rek = request.form.get('no_rekening')
        t_id = request.form.get('telegram_id_mitra')
        layanan_id = request.form.get('layanan_id')

        # 2. Validasi File Upload Foto KTP
        if 'ktp_foto' not in request.files:
            return render_template('register.html', layanan=daftar_layanan, error_pesan="❌ File KTP tidak ditemukan.")
            
        file = request.files['ktp_foto']
        if file.filename == '' or not allowed_file(file.filename):
            return render_template('register.html', layanan=daftar_layanan, error_pesan="❌ Format file salah (Hanya png/jpg/jpeg).")
        ...
        # 3. Operasi Transaksional Pangkalan Data
        ...
        # 4. Pengiriman Payload Notifikasi ke Telegram Admin Thread
        try:
            ...
            message_text = (
                ...
                f"▫️ **Nama:** {nama}\n"
                f"▫️ **NIK:** `{nik}`\n"
                f"▫️ **WA:** [Hubungi (0{wa[2:]})](https://wa.me/{wa})\n"
                f"▫️ **ID Telegram:** `{t_id}`\n"
                f"▫️ **Spesialisasi:** *{nama_layanan_terpilih}*\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                "⚠️ *Silakan cek panel admin bot untuk melakukan verifikasi berkas KTP.*"
            )
            ...
            logger.error(f"Gagal mengirim notif API Telegram: {bot_e}")
